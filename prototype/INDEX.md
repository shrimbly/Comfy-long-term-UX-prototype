...
| 05  | Install gate, fork-on-open, publish to workspace (flowcharts + per-persona walkthroughs)                            | `/prototype/dashboard` → open / right-click a shared workflow     | `components/{WorkflowCard,InstallGateDialog,PublishToWorkspaceDialog,WorkflowContextMenu}.vue`, `composables/{useWorkflowCompat,useWorkflowPublish}.ts` | `decisions/published-workflow-model.md`, `decisions/team-locked-install.md`, `decisions/workspace-install-registry.md`, `concepts/install-journeys.md`, `concepts/install-switcher.md` | in progress |
