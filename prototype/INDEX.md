...
| 06  | Local media as managed references (Tier 1) — **non-final**                                                          | `/prototype/dashboard` (solo-local persona) → sidebar → Media assets | `src/prototype/components/LocalMediaView.vue` (fork of upstream `MediaAssetsView`) + `{RelinkDialog,AddMediaDialog,RemoveFromComfyDialog}.vue` + `stores/mediaReferenceStore.ts` + `composables/usePrototypeAssetsProvider.ts` + `fixtures/localMedia.ts`; gated edits to `MediaAssetCard.vue` / `MediaAssetContextMenu.vue` | open-q `local-media-as-references`, `concepts/local-dashboard-views.md`, `entities/media-file.md`, `decisions/cloud-only-permissions.md`, `decisions/asset-bundling-on-share-is-mandatory.md` | exploration |
